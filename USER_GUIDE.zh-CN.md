...
赛道状况与两个温度默认沿用 Event 的数值，只有当这个 Session 的情况不同时才需要填写，例如晴天的比赛日里有一场 Heat 遇雨。留空即沿用 Event，输入框中的灰色数字就是 Event 的数值。

Session 页面会显示一行状况，例如“湿地 · 赛道 19 °C · 本 Session 单独设定”或“干地 · 沿用 Event”。在地图上查看 Marker 时，显示的是与这个 Session 状况对应的干地或湿地参考笔记。
...
热胎胎压和胎温不会复制。热胎读数是回场后才测量的，若一并复制过来，屏幕上就分辨不出哪些是真正测过的。

新 Run 的圈速、最高转速、圈数、完成状态和 Driver feedback 同样保持空白，避免把上一次的结果误认为本次结果。
...
如果这个 Event 选定了已保存的 Layout，轮胎区域上方会显示这条赛道在相同赛道状况下过往的胎压增量，以及赛道温度与今天最接近的那一次 Run，方便设定冷胎压力时参考。这只是过往记录，**并非推荐值**。正在编辑的这个 Run 本身不会被计入。
...
- 最快圈。
- 平均圈速。
- 最高转速：读自记录仪。它与齿比配合使用，用来判断换齿盘后是否顶到了限转，或者还有转速没用上。
- 名次。

圈速两种写法都可以：`52.400` 这样的秒数，或 `1:02.500` 这样的分秒。两种都会被正确识别。
若填入的内容无法识别为圈速，输入框下方会给出提示：记录仍会保存，但不会计入最快圈与对比。
...
只要任意地方至少存在两个 Runs，就可以点击 **比较 Runs**。不限于同一个 Session，
也不限于同一个 Event：可以拿今天的 Run 与几个月前在同一条赛道的 Run 相比。

1. 在 **第一个 Run** 和 **第二个 Run** 中选择需要比较的 Runs。选项按 Event 与 Session 分组；
   因为不同 Event 的 Run 都可能叫“Run 01”，表头会标明每列来自哪个 Event 和 Session。
2. 页面首先显示两个 Run 各自的赛道状况、赛道温度和环境温度，然后按照成绩、每条轮胎、Chassis setup 和 Driver feedback 分组显示数据。
3. 蓝色背景表示两次 Run 的数据不同。
4. 勾选 **只看差异**，只显示发生变化的项目。
5. **最快圈变化** 显示第二个 Run 相对第一个 Run 的最快圈差值；负数代表第二个 Run 更快。
   如果一个是干地、一个是湿地，会提示“条件不同”：这种情况下的圈速差主要来自路面，而不是 Setup 的改动。
6. 若两个 Run 的前后齿盘都填了，还会显示 **齿比变化**，例如“-7.7% 更长”。
   “更短”意味着加速更好、极速更低，“更长”则相反。
...
Layout 页面下方会根据过往 Run 自动整理两项记录，不需要重新输入：

- **这里用过的齿比**：每组前后齿盘及其齿比、使用过的 Run 数、最快圈、最高转速、赛道状况，以及最近一次使用的日期。
- **这里的胎压增量**：前轮和后轮从冷胎到热胎的胎压增量，以范围和平均值显示，并按赛道状况分组。每组内的 Run 按赛道温度从低到高排列，温度与增量的关系一目了然。

干地和湿地分开显示，因为湿地的胎压增量远小于干地，合在一起的平均值不能代表任何一种情况。赛道状况取自每个 Session 实际的情况，所以晴天比赛日里遇雨的那场 Heat 会归入湿地一组。

胎压记录只描述过去发生的情况，**不会推荐具体胎压**。数据越多，趋势越可靠；每个数字旁边都注明了它来自多少个 Run。

这两项记录只统计选定了这个 Layout 的 Event。只填了赛道名称、没有选 Layout 的 Event 不会被计入，页面会提示有几个这样的 Event。
