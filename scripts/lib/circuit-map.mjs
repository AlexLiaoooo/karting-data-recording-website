...
/**
 * A line across the track at the lap start, and an arrowhead further along showing which way the
 * lap runs. Each is drawn only when it is actually known.
 *
 * The two are independent because they can be known independently. Kart Silverstone has a
 * direction the owner supplied but no start line anywhere in its source, so it gets the arrow
 * without the line; drawing a start line there would invent one.
 */
export function startFinishTick(canvasLoop, { withArrow = true, withTick = true } = {}) {
  ...
  const line = withTick ? [round1(x + nx), round1(y + ny), round1(x - nx), round1(y - ny)] : null;
...
export function layout(loop, { pitGeometry = null, minTurn, mergeGap, withArrow = true, withStartTick = true }) {
  // Both marks are optional and independent; see startFinishTick.
  const origin = loop[0];
  ...
  return {
    ...
    startTick: withStartTick || withArrow ? startFinishTick(canvas, { withArrow, withTick: withStartTick }) : null,
  };
}
